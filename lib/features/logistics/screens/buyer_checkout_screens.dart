...
TransportJob? _jobForOrder(WidgetRef ref, String orderId) {
  try {
    return ref
        ...
        .jobs
        .firstWhere((j) => j.orderId == orderId);
  } catch (_) {
    return null;
  }
}
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const Text(
            'Pay for produce only. Transport is billed separately if you use AgriLink Transport.',
        ...
        children: [
          ...
          Card(
            child: ListTile(
              ...
              subtitle: const Text(
                'Find a transporter — we match a lorry to the farm.',
              ),
          ...
          Card(
            child: ListTile(
              ...
              subtitle: const Text(
                'I will arrange my own vehicle to the farm.',
              ),
              trailing: const Icon(Icons.chevron_right),
              onTap: () {
                ref
                    .read(logisticsProvider.notifier)
                    .chooseOwnTransport(orderId);
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text(
                      'Own transport recorded. Collect from the farm.',
                    ),
                  ),
                );
                context.go('/business');
...
class _DeliveryAddressScreenState extends ConsumerState<DeliveryAddressScreen> {
  ...
  void initState() {
    ...
    _contact = TextEditingController(
      text: existing?.contactPerson ?? profile?.name ?? '',
    );
    _phone = TextEditingController(
      text: existing?.phone ?? profile?.phone ?? '',
    );
    _address = TextEditingController(
      text: existing?.address ?? profile?.address ?? '',
    );
    _city = TextEditingController(text: existing?.city ?? 'Colombo');
    _notes = TextEditingController(
      text: existing?.instructions ?? 'Unload at the rear goods bay.',
    );
  }
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            AppTextField(
              label:
                  profile?.isIndividualBuyer == true
                      ? 'Recipient name'
                      : 'Business name',
              controller: _biz,
              validator: _req,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (order == null || job == null) {
      return const Scaffold(
        body: Center(child: Text('Transport request not found')),
      );
    }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          Text(
            'Required vehicle: ${vehicle.label} (capacity check enforced)',
            style: const TextStyle(fontWeight: FontWeight.w600),
          ),
          Text(
            'Kurunegala → ${job.deliveryCity}  •  ~${job.distanceKm.toStringAsFixed(0)} km',
          ),
          const SizedBox(height: 16),
          if (matches.isEmpty)
            const EmptyStateView(
              ...
              message:
                  'Suitable transporters have been notified. Ask a transporter to accept, then confirm here.',
            )
          else
            ...matches.map((t) {
          ...
          Card(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    job.transporterName ?? 'Transporter',
                    style: const TextStyle(fontWeight: FontWeight.w700),
                  ),
                  Text(
                    'Vehicle ${job.vehicleNumber} • ${job.vehicleType.label}',
                  ),
                  Text('Driver ${job.driverName}'),
                  Text(
                    'Pickup ${job.etaPickup}  •  Delivery ${job.etaDelivery}',
                  ),
                  const SizedBox(height: 8),
                  const Text(
                    'Product payment and transport payment are recorded as separate transactions.',
                  ),
                ],
              ),
            ),
